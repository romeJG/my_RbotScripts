...
{
	//-----------EDIT BELOW-------------//
	public string MapNumber = "9123";
	public string[] EquippedItems = { };
	int[] SkillOrder = { 2, 4, 3, 1 };
	public long SaveStateLoops = 99999999999;
	//-----------EDIT ABOVE-------------//
	...
	public void ScriptMain(ScriptInterface bot)
	{
		...
		bot.Log($"[{DateTime.Now:hh:mm:ss tt}] Bot Start");

		while (!bot.ShouldExit())
		{
			
			String LT = String.Format("{0:n0}", bot.Inventory.GetQuantity("Legion Token"));
			if(LT == "25,000"){
				bot.Log($"[{DateTime.Now:hh:mm:ss tt}] Script stopped you have 25k LT.");
				break;
			}
			bot.Log($"[{DateTime.Now:hh:mm:ss tt}] you have {LT} Legion tokens");
			TempItemFarm("Elemental Dragon Soul", 20, "infirmary", "r2", "Right", 4100, "Zombie Dragon");
			bot.Quests.EnsureComplete(4100);
