...
public class BluuTemplate
{
	...
	int[] SkillOrder = {3,5,1,2};
	public long SaveStateLoops = 9999999999999;
	//-----------EDIT ABOVE-------------//
	...
	public void ScriptMain(ScriptInterface bot)
	{
		...
		bot.Quests.EnsureAccept(3743);
		    ...
		    bot.Log($"[{DateTime.Now:hh:mm:ss tt}] Starting Bot");
		...
		while (!bot.ShouldExit())
		{
			...
			InvItemFarm("Tendurrr The Assistant", 1, "tercessuinotlim", "m2", "Bottom", 3743, "Dark makai");
			bot.Player.Jump("Wait", "Spawn");
			InvItemFarm("Fragment of Chaos", 80, "blindingsnow", "r17", "Left", 3743, "Chaos Gemrald");
            bot.Player.Jump("Wait", "Spawn");
			TempItemFarm("Broken Betrayal Blade", 8, "evilwarnul", "r13", "Left", 3743, "Legion Fenrir");
            bot.Player.Jump("Wait", "Spawn");
		    bot.Sleep(3000);
			SafeQuestComplete(3743);
			int BGotA = bot.Inventory.GetQuantity("Blood Gem of the Archfiend");
			bot.Player.Pickup("Blood Gem of the Archfiend");
			bot.Player.Pickup("Essence of Nulgath");
		    if (BGotA == 69) {
		    bot.Log($"----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------[{DateTime.Now:hh:mm:ss tt}] You now have "+BGotA.ToString()+"/100 Blood Gem of the Archfiend NICE fuck yeah ;)----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------");
		    }else if (BGotA == 100){
		    bot.Log($"----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------[{DateTime.Now:hh:mm:ss tt}] You might wanna stop now cuz u have "+BGotA.ToString()+"/100 Blood Gem of the Archfiend ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------");
		    }else{
		    bot.Log($"----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------[{DateTime.Now:hh:mm:ss tt}] You now have "+BGotA.ToString()+"/100 Blood Gem of the Archfiend-------------------------------------------------------------------------------------------------------------------------------------------------------------------");
		    }
			int esse = bot.Inventory.GetQuantity("Essence of Nulgath");
			if(esse >= 60){
		...
		bot.Log($"[{DateTime.Now:hh:mm:ss tt}] Script stopped successfully.");
		StopBot();
	}
	...
	*/
	...
	breakFarmLoop:
		SmartSaveState();
		bot.Log($"[{DateTime.Now:HH:mm:ss tt}] Completed Farming Loop {SavedState}.");
		FarmLoop = 0;
		goto startFarmLoop;
	...
	*/
	...
	public void SafePurchase(string ItemName, int ItemQuantityNeeded, string MapName, int ShopID)
	{
		...
		while (!bot.Inventory.Contains(ItemName, ItemQuantityNeeded))
		{
			...
			if (!bot.Shops.IsShopLoaded)
			{
				...
				bot.Log($"[{DateTime.Now:HH:mm:ss tt}] Loaded Shop {ShopID}.");
			}
			bot.Shops.BuyItem(ItemName);
			bot.Log($"[{DateTime.Now:HH:mm:ss tt}] Purchased {ItemName} from Shop {ShopID}.");
		}
	}
	...
	maintainCompleteLoop:
		...
		{
			bot.Log($"[{DateTime.Now:hh:mm:ss tt}] Failed to turn in Quest {QuestID}. Logging out.");
			bot.Player.Logout();
		}
		bot.Log($"[{DateTime.Now:hh:mm:ss tt}] Turned In Quest {QuestID} successfully.");
		bot.Quests.EnsureAccept(QuestID);
		bot.Sleep(1000);
	...
	public void StopBot(string Text = "Bot stopped successfully.", string MapName = "yulgar", string CellName = "Enter", string PadName = "Spawn")
	{
		...
		bot.Log($"[{DateTime.Now:HH:mm:ss tt}] Bot stopped successfully.");
	...
	*/
	...
	{
		bot.Log($"[{DateTime.Now:HH:mm:ss tt}] Successfully Saved State.");
	}
	...
	public void SafeMapJoin(string MapName, string CellName, string PadName)
	{
		...
		if (bot.Player.Cell != CellName) bot.Player.Jump(CellName, PadName);
		bot.Log($"[{DateTime.Now:hh:mm:ss tt}] Joined map {MapName}-{MapNumber}, positioned at the {PadName} side of cell {CellName}.");
	}
